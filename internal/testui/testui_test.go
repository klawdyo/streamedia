...
func TestServeUI(t *testing.T) {
	...
	for _, want := range []string{"Streamedia", "/api/upload/init", "/test/webhook", "Players por resolução"} {
...
// webhook via POST e recuperá-lo via GET /test/webhook/events.
func TestReceiveAndListWebhooks(t *testing.T) {
	h := NewHandler()

	// Recebe um webhook.
	payload := `{"video_id":"abc","event":"transcode_complete","status":"ready"}`
	postReq := httptest.NewRequest(http.MethodPost, "/test/webhook", strings.NewReader(payload))
	...
	listReq := httptest.NewRequest(http.MethodGet, "/test/webhook/events", nil)
...
func TestListEventsSince(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		rec := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodPost, "/test/webhook", strings.NewReader(`{}`))
		h.ReceiveWebhook(rec, req)
	}

	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/test/webhook/events?since=2", nil)
...
func TestWebhookBufferEviction(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/test/webhook/events", nil)
	h.ListEvents(rec, req)
